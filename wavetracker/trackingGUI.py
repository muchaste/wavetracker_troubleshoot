...
class SettingsHarmonicGroup(QMainWindow):
    ...
    def init_widgets(self):
        ...
        self.lowTH_fac_W = QLineEdit(
            str(self.low_thresh_factor), self.central_widget
        )
        ...
        self.highTH_W = QLineEdit(
            str(self.high_threshold), self.central_widget
        )
        ...
        self.highTH_fac_W = QLineEdit(
            str(self.high_thresh_factor), self.central_widget
        )
        self.highTH_fac_L = QLabel(
            "high threshold factor", self.central_widget
        )
        self.gridLayout.addWidget(self.highTH_fac_W, 4, 0)
        self.gridLayout.addWidget(self.highTH_fac_L, 4, 1)

        self.freq_tol_fac_W = QLineEdit(
            str(self.freq_tol_fac), self.central_widget
        )
        self.freq_tol_fac_L = QLabel(
            "freq tollerance factor", self.central_widget
        )
        self.gridLayout.addWidget(self.freq_tol_fac_W, 5, 0)
        self.gridLayout.addWidget(self.freq_tol_fac_L, 5, 1)

        self.mains_freq_W = QLineEdit(
            str(self.mains_freq), self.central_widget
        )
        self.mains_freq_L = QLabel(
            "Main frequencies [Hz]", self.central_widget
        )
        self.gridLayout.addWidget(self.mains_freq_W, 6, 0)
        self.gridLayout.addWidget(self.mains_freq_L, 6, 1)

        self.mains_freq_tol_W = QLineEdit(
            str(self.mains_freq_tol), self.central_widget
        )
        self.mains_freq_tol_L = QLabel(
            "Main frequencies tollerance [Hz]", self.central_widget
        )
        self.gridLayout.addWidget(self.mains_freq_tol_W, 7, 0)
        self.gridLayout.addWidget(self.mains_freq_tol_L, 7, 1)

        self.max_divisor_W = QLineEdit(
            str(self.max_divisor), self.central_widget
        )
        ...
        self.min_group_size_W = QLineEdit(
            str(self.min_group_size), self.central_widget
        )
        self.min_group_size_L = QLabel(
            "min. harmonic group size", self.central_widget
        )
        self.gridLayout.addWidget(self.min_group_size_W, 9, 0)
        self.gridLayout.addWidget(self.min_group_size_L, 9, 1)

        self.max_rel_power_weight_W = QLineEdit(
            str(self.max_rel_power_weight), self.central_widget
        )
        self.max_rel_power_weight_L = QLabel(
            "max. rel. power weight", self.central_widget
        )
        self.gridLayout.addWidget(self.max_rel_power_weight_W, 10, 0)
        self.gridLayout.addWidget(self.max_rel_power_weight_L, 10, 1)

        self.max_rel_power_W = QLineEdit(
            str(self.max_rel_power), self.central_widget
        )
...
class SettingsSpectrogram(QMainWindow):
    ...
    def init_widgets(self):
        self.StartTime = QLineEdit(
            "%.2f" % (self.start_time / 60), self.central_widget
        )
        ...
        self.FreqResolution = QLineEdit(
            str(self.fresolution), self.central_widget
        )
        ...
        self.NfftPerPsd = QLineEdit(
            str(self.nffts_per_psd), self.central_widget
        )
        ...
        if self.samplerate:
            self.real_nfft = QLineEdit(
                "%.0f" % next_power_of_two(self.samplerate / self.fresolution),
                self.central_widget,
            )
            self.temp_res = QLineEdit(
                "%.3f"
                % (
                    next_power_of_two(self.samplerate / self.fresolution)
                    * (1.0 - self.overlap_frac)
                ),
                self.central_widget,
            )
            print(
    ...
    def apply_settings(self):
        ...
        self.real_nfft.setText(
            "%.0f" % next_power_of_two(20000.0 / self.fresolution)
        )
        self.temp_res.setText(
            "%.3f"
            % (
                next_power_of_two(20000.0 / self.fresolution)
                * (1.0 - self.overlap_frac)
                / 20000.0
            )
        )
        if self.samplerate:
            self.data_snippet_idxs = int(
                self.data_snippet_sec * self.samplerate
            )
            self.real_nfft.setText(
                "%.0f" % next_power_of_two(self.samplerate / self.fresolution)
            )
...
class EOD_extraxt(QThread):
    ...
    def fill_spec(self):
        ...
        while start_idx <= end_idx:
            ...
            nfft = next_power_of_two(
                self.samplerate / self.SpecSettings.fresolution
            )

            func = partial(
                spectrogram,
                ratetime=self.samplerate,
                freq_resolution=self.SpecSettings.fresolution,
                overlap_frac=self.SpecSettings.overlap_frac,
            )
            ...
            if len(np.shape(self.data)) == 1:
                a = pool.map(
                    ...
                    [
                        self.data[
                            start_idx : start_idx
                            + self.SpecSettings.data_snippet_idxs
                        ]
                    ],
                )
            else:
            ...
            ############################################
            fill_spec_str = os.path.join(
                "/home/raab/analysis/fine_specs",
            ...
            if first_run:
                ...
                fill_spec = np.memmap(
                    ...
                    dtype="float",
                    mode="w+",
                    shape=(len(self.comb_spectra), len(self.tmp_times)),
                    order="F",
                )
                ...
                if len(pre_save_spectra) == 0:
                    ...
                    pre_save_spectra = np.append(
                        pre_save_spectra, self.comb_spectra, axis=1
                    )
                if np.shape(pre_save_spectra)[1] >= 500:
                    ...
                    fill_spec = np.memmap(
                        ...
                        mode="r+",
                        shape=(
                            np.shape(pre_save_spectra)[0],
                            np.shape(pre_save_spectra)[1] + old_len,
                        ),
                        order="F",
                    )
        ...
        np.save(
            ...
            np.array(np.shape(fill_spec)),
        )
        np.save(
            os.path.join(
                "/home/raab/analysis/fine_specs", file_str, "fill_times.npy"
            ),
            self.times,
        )
        np.save(
            os.path.join(
                "/home/raab/analysis/fine_specs", file_str, "fill_freqs.npy"
            ),
            self.spec_freqs,
        )
    ...
    def snippet_spectrogram(self):
        ...
        while start_idx <= end_idx:
            ...
            if create_plotable_spectrogram:
                ...
                plot_spectra = np.sum(self.spectra, axis=0)[
                    self.spec_freqs < comp_max_freq
                ]

                if self.life_plotting:
                    self.spec_ret = plot_spectra[
                        (plot_freqs < 1200) & (plot_freqs > 400)
                    ]
                    self.freq_ret = plot_freqs[
                        (plot_freqs < 1200) & (plot_freqs > 400)
                    ]
                ...
                if not get_spec_plot_matrix:
                    ...
                    self.tmp_spectra = np.zeros(
                        (len(y_borders) - 1, len(x_borders) - 1)
                    )
                    self.tmp_spectra_SCH = np.array(
                        [
                            np.zeros((len(y_borders) - 1, len(x_borders) - 1))
                            for ch in self.channel_list
                        ]
                    )

                    recreate_matrix = False
                    if (self.tmp_times[1] - self.tmp_times[0]) > (
                        x_borders[1] - x_borders[0]
                    ):
                        try:
                            print("da")
                            x_borders = np.linspace(
                                ...
                                int(
                                    (max_x - min_x)
                                    // (self.tmp_times[1] - self.tmp_times[0])
                                    + 1
                                ),
                            )
                        except:
                ...
                for i in range(len(y_borders) - 1):
                    ...
                    for j in range(len(x_borders) - 1):
                        ...
                        t_mask = np.arange(len(self.tmp_times))[
                            (self.tmp_times >= x_borders[j])
                            & (self.tmp_times < x_borders[j + 1])
                        ]
                        f_mask = np.arange(len(plot_spectra))[
                            (plot_freqs >= y_borders[i])
                            & (plot_freqs < y_borders[i + 1])
                        ]

                        if len(t_mask) == 0 or len(f_mask) == 0:
                            continue
                        # print('yay')
                        self.tmp_spectra[i, j] = np.max(
                            plot_spectra[f_mask[:, None], t_mask]
                        )
                        for ch in self.channel_list:
                            self.tmp_spectra_SCH[ch, i, j] = np.max(
                                self.spectra[ch][f_mask[:, None], t_mask]
                            )
            ...
            if self.single_chennel_analysis == True:
                ...
                for ch in self.channel_list:
                    ...
                    # ToDo: error here !!!
                    self.power = [
                        np.array([]) for i in range(len(self.spec_times))
                    ]

                    for t in range(len(self.spec_times)):
                        self.power[t] = np.mean(
                            self.spectra[ch][:, t : t + 1], axis=1
                        )
                ...
                for t in range(len(self.spec_times)):
                    self.power[t] = np.mean(
                        self.comb_spectra[:, t : t + 1], axis=1
                    )
            ...
            non_overlapping_idx = (1 - self.SpecSettings.overlap_frac) * nfft
            start_idx += int(
                (len(self.spec_times) - self.SpecSettings.nffts_per_psd + 1)
                * non_overlapping_idx
            )
            self.times = np.concatenate((self.times, self.tmp_times))
            ...
            if start_idx >= end_idx or last_run:
                ...
                if self.single_chennel_analysis == True:
                    ...
                    for it in range(len(self.fundamentals_SCH)):
                        print(
                            "### tracking Ch: %.0f of %.0f"
                            % (it, len(self.fundamentals_SCH))
                        )
                        ...
                        (
                            ...
                            self.idx_v,
                            self.sign_v,
                            self.a_error_distribution,
                            self.f_error_distribution,
                            self.idx_of_origin_v,
                            self.original_sign_v,
                        ) = freq_tracking_v5(
                            self.fundamentals_SCH[it],
                            self.signatures_SCH[it],
                            self.times,
                            freq_tolerance=self.HGSettings.freq_tol_fac,
                        ...
                        self.all_a_error_distribution.append(
                            self.a_error_distribution
                        )
                        self.all_f_error_distribution.append(
                            self.f_error_distribution
                        )
    ...
    def extract_fundamentals_and_signatures(self, channel=None):
        ...
        if channel == None:
            func = partial(
                harmonic_groups,
                self.spec_freqs,
                low_threshold=self.HGSettings.low_threshold_G,
                high_threshold=self.HGSettings.high_threshold_G,
                min_freq=400,
                max_freq=2000,
                **self.HGSettings.cfg,
            )
            ...
            func = partial(
                harmonic_groups,
                self.spec_freqs,
                low_threshold=self.HGSettings.low_threshold,
                high_threshold=self.HGSettings.high_threshold,
                min_freq=400,
                max_freq=2000,
                **self.HGSettings.cfg,
            )
        ...
        if channel == None:
            if (
                self.HGSettings.low_threshold_G <= 0
                or self.HGSettings.high_threshold_G <= 0
            ):
                self.HGSettings.low_threshold_G = a[0][5]
                self.HGSettings.high_threshold_G = a[0][6]
        elif (
            self.HGSettings.low_threshold <= 0
            or self.HGSettings.high_threshold <= 0
        ):
        ...
        for p in range(len(self.power)):
            ...
            if len(tmp_fundamentals) >= 1:
                f_idx = np.array(
                    [
                        np.argmin(np.abs(self.spec_freqs - f))
                        for f in tmp_fundamentals
                    ]
                )
...
class MainWindow(QMainWindow):
    ...
    def thread_finished(self):
        ...
        self.open_B.setText(
            "open (%.0f/%.0f)" % (self.finisched_count, self.total_file_count)
        )
    ...
    def run_main(self):
        ...
        if ok:
            ...
            if os.path.exists(
                os.path.join(os.path.split(self.filename)[0], "fishgrid.cfg")
            ):
                self.elecs_y, self.elecs_x = fishgrid_grids(self.filename)[0]
                self.elecs_y_spacing, self.elecs_x_spacing = fishgrid_spacings(
                    self.filename
                )[0]
    ...
    def open(self):
        ...
        fd = QFileDialog()
        self.filename, ok = fd.getOpenFileName(
            self, "Open File", "/", "Select Raw-File (*.raw)"
        )
    ...
    def save(self):
        ...
        if self.single_trace_cb.isChecked():
            try:
                np.save(
                    os.path.join(folder, "all_fund_v.npy"), self.all_fund_v
                )
                np.save(
                    os.path.join(folder, "all_sign_v.npy"),
                    self.all_original_sign_v,
                )
                np.save(os.path.join(folder, "all_idx_v.npy"), self.all_idx_v)
                np.save(
                    os.path.join(folder, "all_ident_v.npy"), self.all_ident_v
                )

                np.save(os.path.join(folder, "all_times.npy"), self.times)
                np.save(
                    os.path.join(folder, "all_spec.npy"), self.tmp_spectra_SCH
                )
                ...
                np.save(
                    ...
                    np.array(
                        [
                            self.EodEctractThread.SpecSettings.start_time,
                ...
                folder = os.path.join(
                    "/home/raab/analysis", os.path.split(self.folder)[-1]
                )
            ...
            try:
                ...
                np.save(
                    os.path.join(folder, "sign_v.npy"), self.original_sign_v
                )
...
class LifeEODfUpdate(QThread):
    ...
    def run(self):
        ...
        if self.handle == None:
            (self.handle,) = self.ax.plot(
                t_array, np.hstack(self.eodf), "o", color="k", zorder=2
            )
        else:
            self.handle.set_data(t_array, np.hstack(self.eodf))
        self.ax.set_ylim(400, 1200)
        self.ax.set_xlim(
            self.times[0], self.times[-1] + (self.times[1] - self.times[0])
        )
