...
class ImagePlotWithHist(QWidget):
    ...
    def __init__(self, data, verbose=0, parent=None):
        ...
        self.plot_x_min, self.plot_x_max = 0.0, 1.0
        self.data_x_min, self.data_x_max = 0.0, 0.0
        self.x_min_for_sb = np.linspace(
            0,
            (
                self.data.shape[0]
                - (self.plot_x_max - self.plot_x_min) * self.data.samplerate
            )
            / self.data.samplerate,
            100,
        )
        self.x_max_for_sb = np.linspace(
            self.plot_x_max - self.plot_x_min,
            (self.data.shape[0]) / self.data.samplerate,
            100,
        )
        self.plot_max_d_xaxis = 5.0
        ...
        self.power_hist.sigLevelsChanged.connect(
            lambda: self.v_min_max_adapt_sig.emit(self)
        )
        ...
        self.plot_widgets[0].sigXRangeChanged.connect(
            lambda: DataViewer.plot_xlims_changed(self)
        )
...
class SubplotScrollareaWidget(QScrollArea):
    ...
    def __init__(
        self, plots_per_row, num_rows_visible, data, verbose=0, parent=None
    ):
        ...
        if value == True:
            for plot_widget in self.plot_widgets:
                plot_widget.mouseDoubleClickEvent = (
                    lambda event,
                    p=plot_widget: self.adjust_ylim_to_double_clicked_subplot(
                        event, p
                    )
                )
        else:
    ...
    def adjust_ylim_to_double_clicked_subplot(self, event, plot):
        ...
        doi = self.data[
            int(x0 * self.data.samplerate) : int(x1 * self.data.samplerate)
            + 1,
            plot_idx,
        ]
...
class DataViewer(QWidget):
    ...
    def __init__(self, data=None, cfg=None, verbose=0, parent=None):
        ...
        self.plot_max_d_xaxis = 5.0
        self.plot_current_d_xaxis = 1.0
        self.x_min, self.x_max = 0.0, self.plot_current_d_xaxis
        self.current_data_xrange = (
            0,
            self.plot_current_d_xaxis * 2.0,
            self.plot_current_d_xaxis * 2.0,
        )
        ...
        if cfg:
            ...
            self.v_min, self.v_max = (
                self.cfg.harmonic_groups["min_good_peak_power"],
                -50,
            )
            self.min_freq, self.max_freq = (
                self.cfg.harmonic_groups["min_freq"],
                cfg.harmonic_groups["max_freq"],
            )
        ...
        self.TracesSubPlots = SubplotScrollareaWidget(
            plots_per_row=3,
            num_rows_visible=3,
            data=self.data,
            verbose=self.verbose,
        )
        self.TracesSubPlots.create_subplots(
            fn=pg.PlotCurveItem, xlabel="time [s]", ylabel="ampl. [a.U.]"
        )
        self.TracesSubPlots.update_data_sig.connect(self.plot_update_traces)
        self.TracesSubPlots.scroll_ylim_per_double_click = True
        self.TracesSubPlots.plot_widgets[0].setXRange(
            self.TracesSubPlots.plot_x_min, self.TracesSubPlots.plot_x_max
        )
        self.main_layout.addWidget(self.TracesSubPlots, 0, 0)

        self.SpecsSubPlots = SubplotScrollareaWidget(
            plots_per_row=3,
            num_rows_visible=3,
            data=self.data,
            verbose=self.verbose,
        )
        self.SpecsSubPlots.create_subplots(
            fn=pg.ImageItem, xlabel="time [s]", ylabel="frequency [Hz]"
        )
        self.SpecsSubPlots.update_data_sig.connect(self.plot_update_specs)
        self.SpecsSubPlots.plot_widgets[0].setXRange(
            self.SpecsSubPlots.plot_x_min, self.SpecsSubPlots.plot_x_max
        )
        self.SpecsSubPlots.plot_widgets[0].setYRange(
            self.min_freq, self.max_freq, padding=0
        )
        self.main_layout.addWidget(self.SpecsSubPlots, 0, 0)
        self.SpecsSubPlots.hide()

        self.SumSpecPlot = ImagePlotWithHist(
        ...
        self.SumSpecPlot.plot_widgets[0].setXRange(
            self.SumSpecPlot.plot_x_min, self.SumSpecPlot.plot_x_max
        )
        self.SumSpecPlot.plot_widgets[0].setYRange(
            self.min_freq, self.max_freq, padding=0
        )
    ...
    def plot_update_sumspec(self, obj):
        ...
        if not self.force_update_spec_plot:
            self.Spec.snippet_spectrogram(
                self.data[x_idx_0:x_idx_1, :].T, obj.data_x_min
            )
        self.force_update_spec_plot = False
        obj.plot_handels[0].setImage(
            decibel(self.Spec.sum_spec.T),
            levels=[self.v_min, self.v_max],
            colorMap="viridis",
        )
        obj.plot_handels[0].setRect(
            pg.QtCore.QRectF(
                self.Spec.spec_times[0],
                self.Spec.spec_freqs[0],
    ...
    def plot_update_specs(self, obj):
        ...
        for ch in range(self.data.channels):
            obj.plot_handels[ch].setImage(
                decibel(self.Spec.spec[ch, :, :].T),
                levels=[self.v_min, self.v_max],
                colorMap="viridis",
            )
    ...
    def plot_update_traces(self, obj):
        ...
        y_min = np.min(
            self.data[
                int(obj.plot_x_min * self.data.samplerate) : int(
                    obj.plot_x_max * self.data.samplerate
                )
                + 1,
                :,
            ]
    ...
    @staticmethod
    def plot_xlims_changed(
        cls,
    ):  # ToDo: move this method to the master class; emit signal which then triggers this function... .sigXRangeChanged.connect(self.xlim_changed_signal.emit(self))
        if cls.update_xrange_without_xlim_grep:
            cls.update_xrange_without_xlim_grep = False
        else:
            cls.plot_x_min, cls.plot_x_max = (
                cls.plot_widgets[0].getAxis("bottom").range
            )

        cls.x_min_for_sb = np.linspace(
            0,
            (
                cls.data.shape[0]
                - (cls.plot_x_max - cls.plot_x_min) * cls.data.samplerate
            )
            / cls.data.samplerate,
            100,
        )
        cls.x_max_for_sb = np.linspace(
            cls.plot_x_max - cls.plot_x_min,
            (cls.data.shape[0]) / cls.data.samplerate,
            100,
        )
        ...
        if cls.plot_x_min < 0:
            ...
            cls.plot_widgets[0].setXRange(
                cls.plot_x_min, cls.plot_x_max, padding=0
            )
            ...
            cls.plot_widgets[0].setXRange(
                cls.plot_x_min, cls.plot_x_max, padding=0
            )  # triggers the same function again
        elif (
            (
                cls.plot_x_min
                < cls.plot_x_min - (cls.plot_x_min - cls.data_x_min) * 0.5
            )
            and (cls.plot_x_min > (cls.plot_x_max - cls.plot_x_min))
        ) or (
            cls.plot_x_max
            > cls.plot_x_max + (cls.data_x_max - cls.plot_x_max) * 0.5
        ):
            if cls.verbose >= 1:
                print("\nemit updating data:")
                print(
                    f"data_x_min: {cls.data_x_min:.2f}s; plot_x_min: {cls.plot_x_min:.2f}s"
                )
                print(
                    f"data_x_max: {cls.data_x_max:.2f}s; plot_x_max: {cls.plot_x_max:.2f}s"
                )
    ...
    def add_scrollbar_to_adjust_xrange(self):
        ...
        self.x_scrollbar.sliderReleased.connect(
            lambda: self.update_plot_x_limits_by_scrollbar(
                self.x_scrollbar.value()
            )
        )
    ...
    def define_actions(self):
        ...
        self.Act_spec_nfft_up.triggered.connect(
            lambda: setattr(
                self.Spec, "nfft", int(2 ** (np.log2(self.Spec.nfft) + 1))
            )
        )
        ...
        self.Act_spec_nfft_down.triggered.connect(
            lambda: setattr(
                self.Spec,
                "nfft",
                int(2 ** (np.log2(self.Spec.nfft) - 1))
                if int(2 ** (np.log2(self.Spec.nfft) - 1)) > 16
                else self.Spec.nfft,
            )
        )
        ...
        self.Act_spec_overlap_up.triggered.connect(
            lambda: setattr(
                self.Spec,
                "overlap_frac",
                self.Spec.overlap_frac + 0.05
                if self.Spec.overlap_frac < 0.95
                else self.Spec.overlap_frac,
            )
        )
        ...
        self.Act_spec_overlap_down.triggered.connect(
            lambda: setattr(
                self.Spec,
                "overlap_frac",
                self.Spec.overlap_frac - 0.05
                if self.Spec.overlap_frac > 0.05
                else self.Spec.overlap_frac,
            )
        )
    ...
    def update_plot_x_limits_by_scrollbar(
        self, value
    ):  #  1-100 as set earlier
        for obj in [self.TracesSubPlots, self.SpecsSubPlots]:
            if obj.isVisible():
                ...
                obj.plot_widgets[0].setXRange(
                    obj.plot_x_min, obj.plot_x_max, padding=0
                )
                return
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_S:
            if self.TracesSubPlots.isVisible():
                if (
                    self.SpecsSubPlots.plot_x_min
                    != self.TracesSubPlots.plot_x_min
                ) or (
                    self.SpecsSubPlots.plot_x_max
                    != self.TracesSubPlots.plot_x_max
                ):
                    (
                        self.SpecsSubPlots.plot_x_min,
                        self.SpecsSubPlots.plot_x_max,
                ...
                if (
                    self.SumSpecPlot.plot_x_min
                    != self.SpecsSubPlots.plot_x_min
                ) or (
                    self.SumSpecPlot.plot_x_max
                    != self.SpecsSubPlots.plot_x_max
                ):
                    (
                        self.SumSpecPlot.plot_x_min,
                ...
                f0, f1 = (
                    self.SpecsSubPlots.plot_widgets[0].getAxis("left").range
                )
...
class DataViewerStandalone(QMainWindow):
    def __init__(self, args, parent=None):
        ...
        self.central_widget = QWidget(
            self
        )  # basic widget ; can be exchanged with tabwidget etc.
    ...
    def open_with_file_dialog(self):
        fd = QFileDialog()
        self.file, _ = fd.getOpenFileName(
            self, "Open File", "/home", "*.raw *.wav"
        )
        # embed()
        # quit()
